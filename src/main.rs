...
<p class="note" style="font-size:1.2rem">日本語:<br>Musician Audio「USB DAC PEGASUS」シリーズの次期製品として、R2RのUSB-DACよりも、ローム（ROHM）製フラッグシップD/AコンバーターIC「BD34302EKV」を4基（クアッド構成）搭載して、低ノイズで音質の良い真空管も搭載して、スイッチのONとOFFが可能なモデルを、是非次期製品として発売を希望致しております。<br><br>English:<br>As the next product in Musician Audio's "USB DAC PEGASUS" series, rather than an R2R USB-DAC, we would very much like to see the release of a model equipped with four (quad configuration) ROHM "BD34302EKV" flagship D/A converter ICs, also featuring a low-noise, good-sounding vacuum tube stage that can be switched ON and OFF.</p>
