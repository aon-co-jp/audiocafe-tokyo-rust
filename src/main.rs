...
<p class="note" style="font-size:1.2rem">熱心なAUDIOファンで音質の良い、MUSICIAN AUDIOのPISCES IIIと言うDDC（GustardのDDCのU18よりロスレスのI2Sなので音質が良いとされております。）＋USB-DAC次期製品を希望している者と致しまして、<br>As an avid AUDIO fan hoping for a good-sounding next-generation DDC + USB-DAC — specifically MUSICIAN AUDIO's PISCES III DDC (said to sound better than Gustard's U18 DDC thanks to its lossless I2S) — my request is as follows:</p>
